...
#pragma warning restore CS0618 // Type or member is obsolete
        while (!w.isDone)
            w.MoveNext();
        Debug.Log(w.text);
        callBack(w.text.Replace("：",":").Replace("！","!").Replace("？","?").Replace("（","(").Replace("）",")").Replace("；", ";"));
    }
    ...
    /// <summary>
    /// Dogru formattaki text'de calisiyor, yazim kurali olarak virgulden sonraki karakterin bosluk karakteri olmasi gerek.
    /// OKUNAN FORMAT : -,0,Resting, 5min,Dinleniyor,Ruht sich aus,,,,,Descansando,Se repose,A descansar,Отдыхает,Odpoczywa
    /// "Resting, 5min" birlesik ifade, eski sistemde Resting ve 5min olarak boluyor
    /// </summary>
    static string[] SplitCsvLine(string line)
    {
        var newLine = line;

        // virgulden sonra hemen bosluk karakteri geliyor ise yildiza cevir
        newLine = newLine.Replace(", ", "*");

        // virgulden sonra bosluk gelmiyosa text'i bol - OKUNAN FORMAT GIBI
        var matches = System.Text.RegularExpressions.Regex.Matches(newLine,
            @"(((?<x>(?=[,]+))|""(?<x>([^""]|"""")+)""|(?<x>[^,]+)),?)",
            System.Text.RegularExpressions.RegexOptions.ExplicitCapture);

        // text'i default haline cevir (yildizlari tekrar virgul+bosluk)
        return (from System.Text.RegularExpressions.Match m in matches
                select m.Groups[1].Value.Replace("*", ", ")).ToArray();
    }
